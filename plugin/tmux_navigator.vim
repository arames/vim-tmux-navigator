...
if !exists("g:tmux_navigator_wrap_window")
  let g:tmux_navigator_wrap_window = 0
endif
...
function! s:TmuxAwareNavigate(direction)
  ...
  if s:ShouldForwardNavigationBackToTmux(tmux_last_pane, at_tab_page_edge)
    ...
    if g:tmux_navigator_wrap_window && (a:direction == 'l' || a:direction == 'h')
      if a:direction == 'l'
        let args = 'if-shell -F "#{pane_at_right}" "next-window; select-pane -t 0" "select-pane -R"'
      else
        let args = 'if-shell -F "#{pane_at_left}" "next-window; select-pane -t 0; select-pane -L" "select-pane -L"'
      endif
    else

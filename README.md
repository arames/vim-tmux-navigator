...
``` vim
Plugin 'arames/vim-tmux-navigator'
" Set to 1 if navigating on the rightmost or leftmost pane should switch to the
" next or previous window.
let g:tmux_navigator_wrap_window = 1
```
...
If you have set `g:tmux_navigator_wrap_window`, use the following instead for
'right' and 'left'.

```
bind-key -n C-h if-shell "$is_vim" "send-keys C-h" "if-shell -F \"#{pane_at_left}\" \"previous-window; select-pane -t 0; select-pane -L\" \"select-pane -L\""
bind-key -n C-l if-shell "$is_vim" "send-keys C-l" "if-shell -F \"#{pane_at_right}\" \"next-window; select-pane -t 0\" \"select-pane -R\""
```
